 ...
 */
int bootdef_open(char *bootdef_path) {
 ...
 */
char *bootdef_get_value(char *key) {
    char *bootdef_key;
    char *bootdef_value;
    char *line;
 ...
 */
int bootdef_get_version() {
    char *v1_key = "DistType";

    if( bootdef_get_value(v1_key) != NULL ) {
        return(1);
    } else {
        return(2);
    }
 ...
 */
int bootdef_section_find(char *section_name) {
    char *line;
    char *tok;
 ...
 */
int bootdef_section_get(char **script, char *section_name) {
    char *line;

    singularity_message(VERBOSE, "Attempting to find and return script defined by section %%%s\n", section_name);
    if( bootdef_section_find(section_name) == -1 ) {
